...
export default function ContactOption({ type, description, icon: Icon, action, available, onClick }: ContactOptionProps) {
  return (
    <div className="flex items-center space-x-4 p-4 bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 hover:shadow-md transition-all duration-300 group">
      ...
      <button 
        onClick={onClick}
        className="flex items-center space-x-1 px-3 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-all duration-300 text-sm hover:scale-105 hover:shadow-md"
      >
        <span>{action}</span>
